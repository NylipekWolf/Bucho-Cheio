...
export async function getIngredienteService(request: filtroIngrediente) {
  ...
  try {
    ...
    dados.forEach((item) => {
      ...
      item.vencimento = item.vencimento.toISOString().slice(0, 10);
    }); //Tranforma as datas de vencimento para o padrão "YYYY-MM-DD"
    return dados;
  } catch (err) {
    console.log(err);
    return [];
...
export async function postIngredienteService(request: createIngrediente) {
  try {
    const ingredienteCriado =
      await sql`INSERT INTO bucho_cheio.ingrediente(nome, quantidade, vencimento, fornecedor, preco)
        VALUES (${request.nome}, ${request.quantidade}, ${request.vencimento}, ${request.fornecedor}, ${request.preco}) RETURNING *;`;

    ingredienteCriado[0].vencimento = ingredienteCriado[0].vencimento
      .toISOString()
      .slice(0, 10);
...
export async function putIngredienteService(request: ingredienteRequest) {
  try {
    const testeId =
      await sql`SELECT COUNT(*) FROM bucho_cheio.pedido WHERE id=${request.id};`; //Checagem se id existe
    if (testeId[0].count > 0) {
      ...
      ingredienteModificado[0].vencimento = ingredienteModificado[0].vencimento
        .toISOString()
        .slice(0, 10);
...
export async function deleteIngredienteService(id: number) {
  try {
    const testeId =
      await sql`SELECT COUNT(*) FROM bucho_cheio.ingrediente WHERE id=${id};`; //Checagem se id existe
    if (testeId[0].count > 0) {
      const ingredienteDeletado =
        await sql`DELETE FROM bucho_cheio.ingrediente WHERE id=${id};`;
      return true;
    } else {
      return false;
    }
