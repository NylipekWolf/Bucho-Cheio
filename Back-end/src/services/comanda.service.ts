...
export async function serviceGetComanda(id: number | undefined, status: number[] | undefined, mesa: number | undefined) {
    try{ //Implementar filtro
        const dados = await sql`SELECT * FROM bucho_cheio.comanda ORDER BY id ASC`;
        return dados;
    } catch (err) {
        // console.log(err);
        return [];
